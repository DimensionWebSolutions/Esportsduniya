...
.faq h3{font-size:.95rem;margin:1rem 0 .25rem;color:#fff}
.faq p{color:#aaa;font-size:.9rem}
.facts{display:flex;flex-wrap:wrap;gap:.5rem;list-style:none;padding:0;margin:0 0 1.5rem}
.facts li{border:1px solid #222;border-radius:8px;padding:.5rem .75rem;background:#12121a;font-size:.85rem}
.facts span{display:block;color:#888;font-size:.7rem;text-transform:uppercase;letter-spacing:.05em}
`;
...
function renderKeyFacts(keyFacts) {
  ...
  return `<ul class="facts">${keyFacts.map(f => `<li><span>${escapeHtml(f.label)}</span>${escapeHtml(f.value)}</li>`).join('')}</ul>`;
}

function renderFaqSection(faqs, heading) {
  if (!faqs?.length) return '';
  const items = faqs.map(f => `<h3>${escapeHtml(f.q)}</h3><p>${escapeHtml(f.a)}</p>`).join('');
  return `<div class="faq"><h2>${escapeHtml(heading)}</h2>${items}</div>`;
}
...
export function renderTopicalHubPage(hubKey, matches) {
  ...
  const bodyHtml = `
    ...
    ${renderKeyFacts(hub.keyFacts)}
    ${renderMatchList(matches, hub.sport)}
    ${renderFaqSection(hub.faqs, `${hub.h1.split('—')[0].trim()} — questions fans ask`)}
    <p style="margin-top:1.5rem"><a href="${SITE_URL}/sport/${hub.sport}">All ${escapeHtml(hub.sport)} live scores →</a> · <a href="${SITE_URL}/standings">Points table</a> · <a href="${SITE_URL}/quiz">Daily sports quiz</a></p>`;
  const itemList = buildItemListJsonLd(matches, hub.h1);
  const jsonLd = hub.faqs?.length
    ? { '@context': 'https://schema.org', '@graph': [{ '@type': itemList['@type'], name: itemList.name, itemListElement: itemList.itemListElement }, buildFaqJsonLd(hub.faqs)] }
    : itemList;
  return pageShell({
    title: hub.title,
