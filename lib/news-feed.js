...
/** Map RSS/news articles to highlights reel shape */
export function newsItemsToHighlights(items, limit = 5) {
  return (items || []).slice(0, limit).map((item, i) => ({
    id: i + 1,
    sport: item.category || 'general',
    title: item.title,
    summary: item.metaDescription || item.title,
    matchContext: item.sourceName || '',
    timestamp: typeof item.publishedAt === 'string' ? item.publishedAt : new Date(item.publishedAt).toISOString(),
    url: item.sourceUrl || '',
  }));
}
